...
async def generate_story_endpoint(request: Request, body: StoryRequest):
    ...
    try:
        ...
        # Save story to database and send completion email if user info provided
        story_id = None
        if body.user_id and body.child_profile_id and supabase:
            try:
                logger.info(f"Saving story to database for user {body.user_id}...")
                
                # Collect scene URLs
                scene_urls = [str(page.scene) for page in story_pages if page.scene]
                
                # Prepare story content
                story_content = {
                    "title": body.story_title or story_result.get('title', 'Untitled Story'),
                    "pages": [
                        {
                            "pageNumber": i + 1,
                            "text": page.text,
                            "sceneImage": str(page.scene) if page.scene else None,
                            "audioUrl": str(page.audio) if page.audio else None
                        }
                ...
                story_data = {
                    ...
                    "character_type": body.character_type,
                    "special_ability": body.special_ability,
                    "character_style": body.character_style or "3d",
                    "story_world": body.story_world,
                    "adventure_type": body.adventure_type,
                    "original_image_url": str(body.character_image_url) if body.character_image_url else None,
                    "enhanced_images": body.enhanced_images or [],
                    "story_title": body.story_title or story_result.get('title', 'Untitled Story'),
                    "story_content": json.dumps(story_content),
                    "scene_images": scene_urls,
                    "audio_url": audio_urls if audio_urls else [],
                    "status": "completed",
                    "story_type": "story"
                }
                ...
                if result.data and len(result.data) > 0:
                    ...
                    story_uid = result.data[0].get("uid")
                    logger.info(f"✅ Story saved to database with ID {story_id} (UID: {story_uid})")
                    
                    # Send book completion email
                    try:
                        from email_service import email_service, send_book_completion
                        
                        if email_service.is_enabled():
                            # Get user details
                            user_result = supabase.table("users").select("email, first_name, last_name").eq("id", body.user_id).execute()
                            ...
                            if user_result.data and len(user_result.data) > 0:
                                ...
                                child_result = supabase.table("child_profiles").select("first_name").eq("id", body.child_profile_id).execute()
                                ...
                                if user_email:
                                    ...
                                    frontend_url = os.getenv("FRONTEND_URL", "http://localhost:5173")
                                    preview_link = f"{frontend_url}/story/{story_uid}"
                                    download_link = f"{frontend_url}/api/stories/{story_uid}/download"
                                    ...
                                    await send_book_completion(
                                        ...
                                        character_name=body.character_name,
                                        character_type=body.character_type,
                                        book_title=body.story_title or story_result.get('title', 'Untitled Story'),
                                        special_ability=body.special_ability,
                                    ...
                                    logger.info(f"✅ Book completion email sent to {user_email}")
                                else:
                                    logger.warning(f"No email found for user {body.user_id}")
                            else:
                                logger.warning(f"User {body.user_id} not found in database")
                        else:
                            logger.info("Email service not enabled, skipping book completion email")
                    
                    except Exception as email_error:
                        logger.error(f"Error sending book completion email: {email_error}")
                        # Don't fail the request if email fails
                
                else:
                    logger.warning("Failed to save story to database: No data returned")
            
            except Exception as db_error:
                logger.error(f"Error saving story to database: {db_error}")
                import traceback
                logger.debug(f"Traceback: {traceback.format_exc()}")
